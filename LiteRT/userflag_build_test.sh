...
echo -e "${CYAN}Step 1: Changing directory to /home/LiteRT... ${RESET}"
cd /host_home/LiteRT || { echo -e "${RED}Failed to change directory.${RESET}"; exit 1; }

echo -e "${CYAN}Step 1: Building LiteRT test binary...${RESET}"
...
BUILD_CMD+=" \
  tflite/examples/test:test"

# 명령 실행
eval $BUILD_CMD

echo -e "${CYAN}Step 2: Granting execute permission...${RESET}"
chmod +x bazel-bin/tflite/examples/test/test

echo -e "${CYAN}Step 3: Moving binary to target directory...${RESET}"
mv bazel-bin/tflite/examples/test/test /host_home/LiteRT_benchmark/
